...
function updateKPIs() {
  ...
  if (total > 0) {
    ...
    const dist = `Now: ${nowPct}% | Soon: ${soonPct}% | Later: ${laterPct}% | Hold: ${holdPct}%`;
    document.getElementById('kpiDistribution').innerHTML = `<span style="color: #1E865C;">Now: ${nowPct}%</span> | <span style="color: #FFA500;">Soon: ${soonPct}%</span> | <span style="color: #FF9800;">Later: ${laterPct}%</span> | <span style="color: #E03E36;">Hold: ${holdPct}%</span>`;
  ...
  const conditionalEl = document.getElementById('kpiConditional');
  conditionalEl.textContent = conditionalCount;
  conditionalEl.style.color = '#18B5E7';
}
  ...
  charts.donut = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: ['#1E865C', '#FFA500', '#FF9800', '#E03E36']
      }]
    },
    options: {
      responsive: true,
...
function updateScatterChart() {
  const datasets = [
    { label: 'Migrate Now', data: [], backgroundColor: '#1E865C', borderColor: '#1E865C', pointStyle: 'circle', pointRadius: 6 },
    { label: 'Migrate Soon', data: [], backgroundColor: '#FFA500', borderColor: '#FFA500', pointStyle: 'circle', pointRadius: 6 },
    { label: 'Migrate Later', data: [], backgroundColor: '#FF9800', borderColor: '#FF9800', pointStyle: 'circle', pointRadius: 6 },
    { label: 'Hold', data: [], backgroundColor: '#E03E36', borderColor: '#E03E36', pointStyle: 'circle', pointRadius: 6 },
    { label: 'Conditional Award', data: [], backgroundColor: '#18B5E7', borderColor: '#18B5E7', pointStyle: 'star', pointRadius: 8 }
  ];
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (context) => {
              const record = rfpRecords.find(r => r.ClientName === context.raw.label);
              return [
                context.raw.label,
                `Readiness: ${context.raw.x}`,
                `Business Value: ${context.raw.y}`,
                record ? `MRS: ${record.mrs.toFixed(2)}` : '',
  ...
  charts.bar = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (context) => `Average: ${context.parsed.x.toFixed(2)} / 5.0`
          }
        }
      }
    }
...
function updateExecutiveSummary() {
  ...
  const readinessAssessment = avgMRS >= 4.0 ? 'strong overall readiness with majority of portfolio migration-ready' : avgMRS >= 3.0 ? 'moderate readiness requiring focused improvement efforts' : 'early-stage portfolio requiring significant readiness investments';
  
  const p1 = `The portfolio of ${total} RFPs shows ${nowPct}% ready for immediate migration (Migrate Now), ${soonPct}% requiring near-term action (Migrate Soon), ${laterPct}% in planning phase (Migrate Later), and ${holdPct}% on hold. Average MRS is ${avgMRS}. The distribution indicates ${readinessAssessment}.`;
  
  const legalAssessment = parseFloat(avgL) >= 4 ? 'high legal flexibility enabling conditional awards' : parseFloat(avgL) >= 3 ? 'moderate flexibility with negotiation opportunities' : 'limited flexibility requiring careful contract structuring';
  
  const p2 = `Primary migration drivers include ${topDomain.name} averaging ${topDomain.avg.toFixed(1)}/5.0. Greatest operational friction centers on ${frictionDomain} (${avgO}/5.0). Readiness levels range from ${minR} to ${maxR}, with ${atoComplete} records showing complete ATO and ${rfpRecords.filter(r => r.scores.R >= 3 && r.scores.R < 5).length} in progress. Legal flexibility is ${legalAssessment}, with ${conditionalCount} candidates eligible for conditional award arrangements.`;
  
  const quarter = 'Q' + (Math.floor((new Date().getMonth()) / 3) + 1);
  const businessValueAnalysis = parseFloat(avgB) >= 4 ? 'high strategic impact justifying priority resource allocation' : parseFloat(avgB) >= 3 ? 'solid business value supporting migration investment' : 'moderate business drivers requiring ROI validation';
  
  const p3 = `Prioritize ${conditionalCount} Conditional Award candidates for ${quarter} contract negotiation, targeting POA&M-based interim authorizations. Resource allocation should focus on ${domains[domains.length - 1].name} improvements (currently ${domains[domains.length - 1].avg.toFixed(1)}/5.0), with estimated ROI driven by ${businessValueAnalysis}. Recommended timeline: ${classifications['Migrate Now']} immediate migrations, ${classifications['Migrate Soon']} within 6 months, ${classifications['Migrate Later']} in extended planning. Flag ${rfpRecords.filter(r => r.scores.O >= 4).length} high-friction operational constraints for budget/timeline review.`;
...
function displayUploadPreview() {
  ...
  html += '<table class="results-table" style="font-size: 11px;"><thead><tr><th>Client Name</th><th>Sector</th><th>Regulatory Language</th><th>ATO Status</th></tr></thead><tbody>';
...
function runTests() {
  ...
  html += `<p style="margin-top: 16px; padding: 12px; background: ${passed === tests.length ? 'rgba(30, 134, 92, 0.1)' : 'rgba(224, 62, 54, 0.1)'}; border-radius: 8px;"><strong>Summary:</strong> ${passed}/${tests.length} tests passed ${passed === tests.length ? '✓' : '✗'}</p>`;
